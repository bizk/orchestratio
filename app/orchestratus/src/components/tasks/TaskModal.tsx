...
export function TaskModal({ task, onSubmit, onClose }: { task?: Task; onSubmit: (draft: TaskDraft) => Promise<void>; onClose: () => void }) {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    onSubmit({ Title: title.trim(), Description: description.trim(), Status: status }).catch((err: Error) => { setError(err.message); setSubmitting(false) })
  }
  return <Modal opened onClose={onClose} title={task ? 'Edit task' : 'New ticket'} centered size="lg" radius="lg" classNames={modalClassNames} overlayProps={{ backgroundOpacity: 0.65, blur: 3 }}>
    <form onSubmit={handleSubmit}><Stack gap="sm">
      ...
      <Group className="app-modal-actions" justify="flex-end" pt="md"><Button variant="default" onClick={onClose} disabled={submitting}>Cancel</Button><Button type="submit" loading={submitting} disabled={!title.trim()}>{task ? 'Save changes' : 'Create task'}</Button></Group>
    </Stack></form>
  </Modal>
}
