...
import { useTaskAgentResponse, useTaskConversation, useTaskPullRequests } from '../../queries'
...
export function TaskCard({ task, projectId, onRun, onEdit, isOverlay = false }: { task: Task; projectId: number | null; onRun: (taskId: number) => void; onEdit: (task: Task) => void; isOverlay?: boolean }) {
  ...
  const conversationUrl = useTaskConversation(projectId, task.ID, task.Status).data ?? null
  return <Card ref={ref} className={`task-card${isDragging ? ' task-card-dragging' : ''}${isOverlay ? ' task-card-overlay' : ''}`} data-status={task.Status} padding="md" withBorder>
    ...
    {task.Status === 'in_progress' && <>{conversationUrl && <Group gap={4} mt="md" wrap="nowrap"><Anchor className="conversation-link" href={conversationUrl} target="_blank" rel="noreferrer" size="sm" onPointerDownCapture={(event) => event.stopPropagation()}>Open Hands conversation <span aria-hidden="true">↗</span></Anchor></Group>}<div className={`latest-agent-response${agentResponse ? ' latest-agent-response-ready' : ''}`}><Text className="pull-request-label" size="xs" tt="uppercase">Latest agent response</Text>{agentResponse ? <Text className="latest-agent-response-text" size="sm" c="gray.3" mt={5}>{agentResponse}</Text> : <Text size="sm" c="dimmed" mt={5}>Waiting for the agent’s response.</Text>}</div><div className={`pull-request-panel${pullRequests.length > 0 ? ' pull-request-panel-ready' : ''}`}>{pullRequests.length > 0 ? <Group gap="xs" wrap="nowrap" justify="space-between"><Text className="pull-request-label" size="xs" tt="uppercase">Pull request ready</Text>{pullRequests.some((pullRequest) => pullRequest.merged) && <Badge size="xs" color="grape" variant="light" leftSection={<span aria-hidden="true">⇅</span>}>Merged</Badge>}</Group> : <Group gap="xs" wrap="nowrap"><span className="pull-request-pulse" aria-hidden="true" /><Text size="sm" c="gray.3">Waiting for a pull request</Text></Group>}{pullRequests.map((pullRequest) => <Anchor key={pullRequest.number} className="pull-request-button" href={pullRequest.url} target="_blank" rel="noreferrer"> <span className="pull-request-icon" aria-hidden="true">⇄</span><span className="pull-request-button-text">View PR #{pullRequest.number}</span><span className="pull-request-arrow" aria-hidden="true">→</span></Anchor>)}</div></>}
