...
export function TasksView({ onHeaderActionChange }: { onHeaderActionChange: (action: ReactNode) => void }) {
  ...
  const handleCreate = async (draft: TaskDraft) => { await createTask.mutateAsync(draft); toast.success('Task created') }; const handleUpdate = async (draft: TaskDraft) => { if (!taskToEdit) return; await updateTask.mutateAsync({ taskId: taskToEdit.ID, task: draft }); toast.success('Task updated'); setTaskToEdit(null) }; const handleRun = async (opts: { agentId: string; repositoryName: string; branchName: string; llmModel: LLMModel }) => { if (runTaskId === null) return; await runTask.mutateAsync({ taskId: runTaskId, ...opts }); toast.success('Task started'); setRunTaskId(null) }
  ...
  return <Stack gap="lg">{error && <Alert color="red" title="Unable to load data">{error.message}</Alert>}<TaskBoard tasks={tasks} projectId={selectedProjectId} onRun={setRunTaskId} onEdit={setTaskToEdit} />{newTask && <TaskModal onSubmit={handleCreate} onClose={() => setNewTask(false)} />}{taskToEdit && <TaskModal key={taskToEdit.ID} task={taskToEdit} onSubmit={handleUpdate} onClose={() => setTaskToEdit(null)} />}{runTaskTarget && <RunTaskModal task={runTaskTarget} repositories={repositories} onSubmit={handleRun} onClose={() => setRunTaskId(null)} />}</Stack>
}
