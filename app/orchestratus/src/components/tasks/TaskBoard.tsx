...
function groupByStatus(tasks: Task[]): Record<Status, Task[]> { const grouped: Record<Status, Task[]> = { backlog: [], in_progress: [], blocked: [], review: [], completed: [] }; tasks.forEach((task) => (grouped[task.Status] ?? grouped.backlog).push(task)); return grouped }
